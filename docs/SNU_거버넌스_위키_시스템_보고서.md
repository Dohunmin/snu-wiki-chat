...
> ⚠️ **2026-07-24 정합성 주의**: 이 문서는 스냅샷(작성 시점 위키 4~9개 기준)입니다. 현재 시스템은 위키 12+(WIKI_MAP)·에이전트 40(38 active)·페이지타입 8종(policy_document 포함)·TOTAL_CHUNK_BUDGET 22로 확장됨. **현행 권위 문서 = [CLAUDE.md](../../CLAUDE.md) + data/agents.config.json**. 아래 수치·위키셋은 역사적 기록으로 읽을 것.
